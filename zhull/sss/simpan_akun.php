...
//menampung variabel yg di POST dari form registrasi
 ...
 $query="insert into user (Email,Password,Nama,Nomor_Telp,TTL,Alamat,Kode_Pos,Jenis_Kelamin)values('$email','$password','$nama','$notelp','$ttl','$alamat','$kode','$kelamin')";
 $exe=mysql_query($query, $conn); //eksekusi query insert ke database
 
//proses setelah eksekusi
if ($exe){
    echo "<script>location.replace('registration_success.php')</script>";
}else{
